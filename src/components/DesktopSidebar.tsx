    ...
    <AnimatePresence mode="wait">
      {!isHidden && (
        <motion.aside
          ...
          animate={{ x: 0, opacity: 1, width: currentWidth }}
          exit={{ x: -SIDEBAR_WIDTH, opacity: 0 }}
          transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
          onMouseEnter={() => isCollapsed && setIsHovered(true)}
          onMouseLeave={() => setIsHovered(false)}
          className="fixed left-0 top-0 bottom-0 z-40 hidden lg:flex flex-col p-4"
        >
          ...
          <div className="h-full w-full glass-card rounded-[2.5rem] border-white/5 flex flex-col overflow-hidden relative shadow-2xl">
            ...
            <div className="flex-1 px-3 py-4 overflow-y-auto custom-scrollbar relative z-10">
              <div className="mb-6">
                {showExpanded && (
                  <p className="text-[10px] font-black text-muted/50 uppercase tracking-[0.25em] mb-4 ml-4">
                    Master Hub
                  </p>
                )}
            ...
            <button
              ...
              className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-12 rounded-full bg-[#121822] border border-white/10 flex items-center justify-center text-muted hover:text-white hover:bg-white/10 transition-all z-50 group/toggle"
            >
              {isCollapsed ? (
